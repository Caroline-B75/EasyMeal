...
Version : 2.0 (architecture unifiée — draft et actif = même modèle Menu)
...
Il s'agit du MÊME modèle `Menu`, avec un champ `status` (:draft ou :active).
...
- **Menu(status: :active)** : menu finalisé avec liste de courses générée.
...
**Note** : sur un menu actif, chaque action unitaire est persistée immédiatement en base. Le bouton "Enregistrer mes modifications" déclenche uniquement la régénération de la liste de courses, pas la sauvegarde des repas (déjà faite).
...
resources :menus do
  member do
    post :activate           # draft → active (UC1)
...
**MenusController** : add_random_meal, replace_meal, regenerate_grocery
